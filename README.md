...
- Step 1 **SOUND DESIGN** の固定11音色ボタンを、**7グループ・45音色**のライブラリへ拡張。
- グループは **実楽器 / PCM、Pad / Atmosphere、Keys / Organ、Lead / Brass、Pluck / Bell、Bass、Strings / Voice**。カテゴリで絞り込み可能。
- 音色名や説明文を対象にした検索欄を追加し、候補数が増えても横一列へ増え続けないUIへ変更。
- ライブラリの音色を選んだ場合も、対応する自然言語プロンプトを入力欄へ入れて既存の「音色を生成」経路を使うため、自由入力と別の音源ロジックを作らない。
- 自然言語解析を「最初に一致した音色カテゴリ + 少数の補正」から、**明暗・暖冷・硬軟・Attack・長さ・空間・広がり・Air/Noise・Metal/Wood・Organic/Digital・Rough/Clean・Thickness・Percussive** などの連続した音色軸へ拡張。
- `ベル + パッド` のように複数カテゴリを同時に指定した場合、主カテゴリだけでなく副カテゴリのAttack / Sustain / Transient等も反映。
- 「ピアノのようなPad」「ギター弦の質感を少し混ぜたBell」のような表現では、`ピアノ` / `ギター` を専用音源への即時切替ではなく **PCM再合成の参照元ヒント**として扱うよう改善。
- 一方、`グランドピアノ`、`エレキギター`、`フレットレスベース`、`ドラムセット`、`DXエレピ` のような明示的な楽器指定は従来どおり専用PCM/FMエンジンを優先。
- `氷 / 鋼 / ガラス / 木質 / 煙 / 霧 / 風` などの比喩表現も、Brightness / Metal / Wood / Air / Space等のboundedな再合成パラメータへ反映。
- 生成値は引き続き既存Patch validatorでClampし、外部Preset／第三者録音／実行コード／ランタイムのネットワーク取得は行わない。
- Native VST3 HostのC++は変更していないため、v0.9.1への更新後に `build_vst3_host.cmd` を再実行する必要はありません。

### v0.9.1 SOUND DESIGN の使い方

1. 自由入力欄へイメージを文章で書くか、音色ライブラリのカテゴリを選択します。
2. 必要なら検索欄へ `木質`、`ベル`、`暖かい`、`暗い` などを入力して候補を絞ります。
3. ライブラリ候補を押すと、その候補を表す自然言語が入力され、そのまま音色を生成します。
4. 自由入力では「音色カテゴリ + 材質感 + Attack + 長さ + 空間」のように複数の特徴を書くほど差が出やすくなります。

例:

```text
ギター弦の木質感を少し残したベル。
アタックは硬く短いが、遠いホールに透明な余韻が長く残る。
```

```text
息を含む柔らかいクワイアとストリングスの中間。
暗く暖かく、ゆっくり立ち上がり、広い空間に長く伸びる。
```
...
v0.9.1以降、抽象的な音色要求は主にPCM Spectral Resynthesisへ送ります。自然言語から実行コードやDSPコードを生成するのではなく、音色カテゴリに加えて複数の意味軸を評価し、**PCM参照元・Morph・倍音数・Brightness・PCM Body/Transient・Detune・Noise・ADSR・Delay**という有限のPatch値へ変換し、既存validatorでClampします。
...
`暖かい / 冷たい`, `明るい / 暗い`, `硬い / 柔らかい`, `速い / 遅いAttack`, `短い / 長い`, `近い / 遠い`, `広い / 狭い`, `metallic / 金属`, `woody / 木質`, `airy / 息`, `rough / ざらつき`, `organic / 有機的`, `太い / 細い` などを組み合わせると、各軸の強さをPCM参照元や再合成パラメータへ限定範囲内で反映します。`ベルとパッドの中間` のような複合カテゴリも、主カテゴリだけでなく副カテゴリの特徴を残します。

ランタイム中にインターネットへ接続して音色データを取得する仕組みではありません。現在の自然言語解析はローカルで決定論的に行うため、生成AIによる音声生成そのものではありませんが、v0.9.0までの単一カテゴリ＋単純キーワード補正より複合表現を反映しやすくしています。
