...
自然言語で楽器・音色・雰囲気を指定すると音源方式まで選択してPatchを生成し、その場で演奏できるローカル実行型ソフトウェア音源です。鍵盤／PCキーボード／Web MIDI／サンプル演奏に加え、鼻歌からMIDIライクなメロディーを作成できます。v0.7.0では鼻歌の音程・タイミング自動補正、楽譜表示、Windows VST3 Instrumentホストを追加し、v0.8.0ではVST3再ロード時のPCキー復旧、VST3本体Editor、音色バリエーション拡張を追加しました。
