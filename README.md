...
- 全パートがVST3音源の曲では、曲のノートを再生前にNative Hostへ渡します。Hostの音声コールバックが共通のサンプル位置を進め、VST3へのノート予約、フリーズ済みWAVのミックス、ループを同じ音声デバイスと時計で行います。再生位置表示はNative Hostの位置を取得します。ブラウザの周期タイマーから演奏中のVST3ノートを送る必要がありません。
- 内蔵音源またはReference調整音源が有効な曲と、個別クリップ試聴では、従来のブラウザ併用再生を維持します。これらの音源まで一つのネイティブ音声エンジンへ移す段階は未実装です。VST3自体のDSP負荷やPCの能力を超える音切れは今回の変更で解消できません。フリーズ録音は従来どおり実時間で行い、Project JSONにはフリーズ音声を保存しません。
- Native Hostを更新しました。アプリを終了し `build_vst3_host.cmd` で再ビルドしてから `start_synth.cmd` で起動し、ブラウザを再読み込みしてください。VST3診断のHost負荷と音切れ候補で変更前後を比較できます。
