...
- VST3がホストへ報告する出力遅延を曲の絶対時刻予約で先行補正します（最大0.5秒、ライブ鍵盤は対象外）。各パートのVST3音源欄にある「このトラックの発音診断」で報告遅延、予約遅延の件数と最大値、Host負荷、音切れ候補を確認できます。予約遅延が0件で音だけ遅い場合は、プラグイン内の音色のアタックやサンプルの立ち上がりを確認してください。CPU全体の使用率が低くても、一回の音声処理が締切を超えることはあります。
- ポップ、フュージョン、バラードのサンプル曲では、メロディーを各小節のコード構成音に合わせて声部進行させ、4種類のモチーフで単調な分散和音を減らしました。すでにJSONに保存した曲は変更されません。新たにサンプル曲を適用すると更新後のメロディーを使用します。
- アプリを終了して `build_vst3_host.cmd` でNative Hostを再ビルドし、ブラウザを更新してください。VST3の内部で報告されない音色のアタック遅延は自動補正できません。

## v0.18.8 の主な変更

- フリーズなしで複数VST3を鳴らす際、Native Hostの音声周期と処理ブロックを512から1024フレームへ拡張しました。瞬間的な処理の集中に余裕を与え、音声コールバックとプラグイン処理の呼び出し回数を減らします。予約済みMIDIのブロック内発音位置は維持します。
- アプリを終了し、`build_vst3_host.cmd` でNative Hostを再ビルドしてから起動してください。フリーズなしの鍵盤演奏では遅延が約10.7 ms増える場合があります。4音源の合計処理が周期に収まらない場合はフリーズか軽いプリセットが必要です。「VST3診断」の「ホスト負荷」と「音切れ候補」の演奏前後の値で確認できます。

## v0.18.7 の主な変更

- Project JSONを読み込むと、曲名の下の操作欄に「読込ファイル: ファイル名」を表示します。曲名を編集しても、どのJSONを読み込んだか確認できます。読み込み前は「なし」です。
- 同じ長さ・サンプルレート・ステレオ形式のVST3フリーズ音声は、再生前に一度だけ合成し、ブラウザで1本の音声として再生します。音量は個別音声の合計を保持し、既存のマスター処理を通します。形式が異なる音声は従来どおり別々に再生します。すべてフリーズ済みの曲では、不要なノート予約タイマーも動かしません。
- 更新後はアプリとブラウザを再起動してください。Native Hostの再ビルドは不要です。Windows実機での音切れ解消は確認が必要です。

## v0.18.6 の主な変更

- VST3音源4つに内蔵音源を足したときの音切れ対策として、内蔵FM・PCM音源の再生終了時に音声ノードを解放します。ピアノ／ギターのFactory PCMは曲が始まる前に生成し、ギターのアンプ歪み計算も音色ごとに再利用します。
- 更新後はアプリを再起動してブラウザを再読み込みしてください。Native Hostの変更はなく再ビルド不要です。4つのVST3本体の音声処理がデバイスの期限を超えている場合、内蔵音源側の改善だけでは解消できません。演奏前後でVST3診断の「音切れ候補」を比較してください。

## v0.18.5 の主な変更

- VST3を含む曲とクリップの再生は、ノートを先にNative Hostへ送って共通の音声フレーム番号に予約します。ブラウザのタイマーやHTTP到着の遅れが、予約後の発音時刻を動かすことを避けます。曲中のVST3ノートが1024件を超える場合や古いNative Hostでは従来の0.8秒先読みを使用します。
- 各パートの「フリーズ（実時間）」と「フリーズ解除」は内蔵音源・Reference・VST3で同じトラック行に表示します。共有中のVST3は共有を解除してからフリーズします。
- Windowsでアプリを終了し、`build_vst3_host.cmd` でNative Hostを再ビルドして起動してください。ブラウザも再読み込みしてください。VST3診断の「音切れ候補」が演奏中に増える場合は、4音源の処理が音声デバイスの期限を超えています。その場合はフリーズ、軽いプリセット、音源数の調整が必要です。Native Hostの予約改善だけでプラグインDSPの負荷は減りません。

## v0.18.4 の主な変更

- 各内蔵音源・CD/Reference調整済み音源のトラック行で「フリーズ（実時間）」を押すと、そのパートの曲と2秒の余韻を共有AudioContextから録音します。完了後は録音済み音声を再生し、通常再生中の音源処理を省けます。「フリーズ解除」でノート編集と通常演奏へ戻せます。
- 保存したProject JSONにはこれらのフリーズ音声も含まれ、再読込で復元します。音色パッチ、クリップ、BPM、音源、音量などが一致しない音声は拒否します。従来のVST3フリーズ付きJSONも読み込めます。ブラウザ再読み込みで更新してください。Native Hostの再ビルドは不要です。
- 録音は曲の長さに応じた実時間を要し、最大42秒・512ノート／パートです。録音中はほかの演奏を避けてください。AudioWorkletに対応しないブラウザでは画面にエラーを表示します。Referenceの元のCD音声やマイク音声はJSONへ保存せず、調整済み内蔵音源の出力だけを収録します。

## v0.18.3 の主な変更

- 7パートの内蔵音源再生で音切れする要因の一つだった、音符ごとのディレイ循環接続を発音終了時に切断します。ディレイ量が0ならディレイ回路自体を作りません。シンセとリシンセに適用します。
- 更新後はアプリを再起動し、ブラウザを再読み込みしてください。Native Hostの再ビルドは不要です。Windows実機の音声処理余裕や、ほかの原因による音切れは環境ごとの確認が必要です。

## v0.18.2 の主な変更

- VST3 の同じ音程が同じインスタンス内で重なっても、曲・フリーズ録音・鍵盤の発音と消音に対応する note ID を付けます。トラック別インスタンスとMIDIチャンネルの割当は従来どおりです。
- Project JSON の保存時にフリーズ音声を圧縮して含め、読み込み後も再録音せず再生できます。保存済みのVST3音色・MIDIチャンネル・ノート・BPMに結び付けて検証します。設定が一致しない音声や壊れたデータは読み込みません。以前のJSONもそのまま読み込めます。最大42秒／パート、音声合計96 MiB、JSON最大160 MiBです。大きな曲では保存・読込に時間が掛かり、JSONファイルも大きくなります。
- アプリを終了し、`build_vst3_host.cmd` で Native Host を再ビルドしてから起動し、ブラウザを再読み込みしてください。VST3側が note ID を解釈する場合に個別のノート消音が効きます。VST3が note ID を無視する場合の同音重複動作はプラグインに依存します。CPU負荷やテンポの問題への対策は含みません。

## v0.18.1 の主な変更

- 7パートの内蔵音源再生で、同じ高さの音が別トラックや別ノートにある場合も発音と消音を独立して管理します。従来どおり「全パート再生」で確認できます。
- CC0 PCM 音源は再生前に読み込むため、最初のノートでも発音と消音の予約が対応します。更新後はブラウザを再読み込みしてください。Native Host の再ビルドは不要です。
- VST3 はトラックごとにネイティブ音源インスタンスを分けています。この時点では同じインスタンス内の note ID は未対応で、v0.18.2 で対応しました。
