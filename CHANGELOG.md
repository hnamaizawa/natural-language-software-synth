...
- 曲全体のVST3絶対時刻予約で、各プラグインが報告する出力遅延を最大0.5秒まで先行補正。トラック別発音診断に報告遅延、予約が期限後に届いた件数と最大遅延、ホスト負荷と音切れ候補を表示。
- 8小節の3種類のサンプル曲のメロディーをコードトーンで声部進行させ、毎小節同じ分散和音を繰り返さないモチーフへ更新。強拍のアボイドノートを避ける。
- Native Host再ビルドとブラウザ再読み込みが必要。音色固有のアタックはプラグイン報告遅延とは別。

## v0.18.8

- ライブ再生する複数VST3のNative Host音声周期・VST3処理ブロックを512から1024フレームへ拡張。4音源の一時的なDSP負荷に対する処理余裕を増やし、コールバック回数を半減。MIDIイベントはブロック内のサンプル位置で引き続き処理。
- Native Hostの再ビルドが必要。ライブ鍵盤演奏の遅延は約10.7 ms増える場合がある。4音源の処理時間の合計が音声周期を超える場合はフリーズや軽いプリセットが必要。

## v0.18.7

- Project JSON読込後、ファイル名を曲名とは別に表示。読み込み成功時のみ更新。
- 形式の揃った複数のVST3フリーズ音声を再生前に1本へ合成し、全フリーズ再生時は不要なノート予約タイマーを停止。ミックスの浮動小数点ヘッドルームを保持。

## v0.18.6

- 内蔵FM、PCMドラム、ベース、ピアノ、ギター、ライセンスPCMの発音終了後に音声ノードを切り離し、繰り返し再生による不要なグラフ残留を抑制。
- ピアノ／ギターのFactory PCMを曲再生前に準備し、ギターのアンプ歪みカーブを再利用。VST3と内蔵音源の同時再生中に発生するブラウザ側の瞬間的な負荷を削減。

## v0.18.5

- VST3の曲ノートをNative Host共通の音声フレーム時計に再生前予約し、ブラウザタイマーとHTTP到着時刻の揺れを削減。大きい曲や旧Native Hostでは従来の先読みへフォールバック。
- Native Hostの予約キュー上限で古いノートを黙って破棄しないようにし、超過時はエラーを返す。
- 各パートのフリーズボタンを音源共通の場所へ統一。Windows Native Host再ビルドとブラウザ更新が必要。

## v0.18.4

- 内蔵音源とCD/Reference調整済み内蔵音源をトラック別に共有AudioContextの音声フレームで正確に実時間フリーズし、再生時は音源演算を録音済み音声に置換。
- Project JSONにフリーズ音声を保存・復元。内蔵音源系は音色パッチも照合し、VST3旧形式との互換性を維持。
- ブラウザキャッシュを更新。Native Host変更なし。

## v0.18.3

- 内蔵シンセ／リシンセのゼロミックス時に不要なディレイ回路を作らず、発音終了時にディレイのフィードバック接続を切断。長時間のマルチトラック再生で不要なAudioNodeが増え続ける経路を修正。
- ローカルの静的アセットをno-storeで配信し、更新後に古いJavaScriptが残ることを防止。

## v0.18.2

- VST3 ノートイベントに発音・消音が一致する有界な note ID を付与。7パート再生、フリーズ録音、通常の鍵盤演奏で同音重複を区別。
- Python ブリッジと Windows Native Host の単発／一括イベントに note ID を伝搬し、停止時に有効な note ID を消音。古いイベント形式は ID -1 として受け付ける。
- Native Host の再ビルドとブラウザ再読み込みが必要。
- フリーズ録音を16-bit PCMに変換・gzip圧縮してProject JSONへ保存し、読込時にトラック設定を検証して復元。旧JSONも読込可能。サイズ上限を設ける。

## v0.18.1

- 内蔵音源のアレンジメント再生で、同じ音程を鳴らす別トラック・別ノートが発音管理を共有しないよう修正。ピアノ、ギター、PCM、FM、リシンセ音源にも適用。
- CC0 PCM は再生開始前に読み込み、発音と消音の予約が同じノートを指すように修正。変更したブラウザ用スクリプトのキャッシュを更新。
- VST3 ネイティブホストのノート ID と同一音程の重複管理は今回の変更対象外。

## v0.18.0

- AudioContext時計で再生位置を追跡し、タイムラインの縦線と小節・拍表示を追加。
- Cメジャー／Aマイナー／Gメジャーの各8小節サンプル曲を追加。全7パートの共通コード進行に沿うノートを生成。
- ブラウザキャッシュを更新。Native Host変更なし。

## v0.17.0

- 曲長を最大64小節へ拡張し、小節目盛り、曲内の再生開始位置、ループ開始・終了を追加。旧Project JSONは4小節として読み込む。
- クリップの開始・長さ入力とノート強さ編集を追加。選択クリップの試聴範囲とフリーズ音声の途中再生位置を修正。
- ブラウザキャッシュ識別子を更新。Native Hostの変更なし。

## v0.16.0

- VST3 Component/Controller の状態をサイズ制限付きで Project JSON に保存・復元。保存済みパスによる再検索、パート割当・共有・音色の自動復元を追加。
- タイムラインで空クリップ作成・複製・削除・移動、ピアノロールの Note Clip 編集、量子化グリッド、音域切替、削除、Undo/Redo を追加。
- マルチパート再生の先読み時間を0.8秒へ拡張。Native Host の再ビルドが必要。

## v0.15.1

- Native Host の音声周期と VST3 処理ブロックを 512 フレームに統一。複数音源時の呼び出し回数を減らし、処理時間の余裕を増やす。
- MIDI 予約イベントの処理で毎ブロックの一時ベクタ確保・stable_partition を除去し、事前確保した領域を再利用。
- Native Host 再ビルドが必要。レイテンシとプラグイン DSP 負荷に関する限界を README に記載。

## v0.15.0

- VST3 のロード直後に必須だったフリーズ解除呼び出しを撤去し、フリーズ中の既存インスタンスの再接続時だけサーバーで再開。
- パートごとの VST3 準備エラーを分離し、内蔵音源とほかの VST3 の再生を継続。エラーの理由を画面に表示。
- ブラウザのキャッシュ識別子を更新。Native Host の再ビルドは不要。

## v0.14.9

- VST3パートの実時間フリーズ音声をローカルのNative Hostで作成し、再生時は単一AudioContext上のBufferSourceを利用して対象音源を休止。
- 同一VST3のパート同士を明示的に共有する設定を追加し、独立音色と共有音色をパート単位で切り替え可能にした。
- ループ境界の先読み予約と固定した音声クロック基準での開始時刻を追加。無音が0.5秒続く音源の早期休止を追加。音の余韻が続く場合は従来の4秒上限を維持。
- Native Host再ビルド、ブラウザキャッシュv0.14.9への更新が必要。

## v0.14.8

- パート行へ個別の VST3 検索・選択、MIDI チャンネル、ロード、本体画面、パラメータ調整を追加。選択中の別パートや単体演奏用インスタンスへの誤操作を防止。
- VST3 本体画面とパラメータ API をパート固有のインスタンス ID へ送信。

## v0.14.7

- VST3をトラック固有のNative Hostインスタンスへロードし、STEP 2のプラグイン変更や単一クリップ試聴から他トラックの音源状態を保護。
- トラック別Native EditorのエンドポイントにインスタンスIDを通し、SSD5キット／Presetを各トラックで編集可能にした。
- MIDIチャンネルの自動空き番号割当と手動上書き、Project JSONでの保存／再読込を追加。
- 再生中の休符で先読みスケジューラの空回りを抑え、Native Hostの単一音源再生時に不要なミキサー中間バッファを省略。
- Web Runtime／CSSキャッシュ識別子をv0.14.7へ更新。Native Hostの再ビルドが必要。

## v0.14.6

- 選択トラックごとのVST3プラグイン一覧と検索ボタンをTRACK SOUNDへ追加し、複数の異なるVST3を各パートへ割り当て可能にした。
- 再生開始時の全ノート一括生成を約0.45秒の先読み予約へ変更し、100ms単位で負荷を分散。
- 同じVST3を使う複数トラックのイベントをインスタンス単位で集約し、1024件以内で一括送信。
- Runtimeキャッシュ識別子をv0.14.6に更新。Native Hostの再ビルド不要。

## v0.14.5

- 同一プラグインIDを割り当てたVST3トラックを1つの共有インスタンスへ集約し、重複する音声処理を削減。
- STEP 2でロード／設定済みのメインVST3インスタンスをマルチトラックから再利用し、SSD5のキット設定を維持。
- VST3 MIDIチャンネル「自動」でドラムトラックをChannel 10へ統一し、個別試聴とアレンジ再生の経路差を解消。
- 音源変更時の旧共有インスタンス解放と、共有先ごとに1回だけ行うイベントクリアを追加。
- Web Runtimeのキャッシュ識別子をv0.14.5へ更新。Native Hostの再ビルドは不要。

## v0.14.4

- トラックごとのNative Hostプロセスと音声デバイスを廃止し、単一プロセス／単一音声出力で最大24個の独立VST3インスタンスを処理。
- Native Host内部にステレオミキサーを追加し、各VST3出力を1つのWindowsオーディオストリームへ合成。
- 発音終了後の余韻時間を確保したうえで、無音VST3インスタンスの`process()`呼び出しを自動休止。
- アレンジのVST3 Note On／Offをトラック単位のboundedバッチとして送信し、ブラウザー／HTTP通信負荷を削減。
- Native通信プロトコルをv4へ更新し、全コマンドをboundedトラックインスタンスIDでルーティング。
- Web Runtimeのキャッシュ識別子をv0.14.4へ更新。Native Hostの再ビルドが必要。
...
- トラックPatchを事前検証・キャッシュし、ノートごとのvalidatorとPatch UI処理を除去。
- アレンジ再生中の鍵盤／ドラム発音DOM表示を停止。
- 各トラック行に内蔵／CD・Reference調整／VST3の独立音源セレクターを追加。
- VST3予約イベントを時刻順共有キューへ集約し、同時再生時のブラウザタイマー数を削減。
...
- トラック単位の内蔵プリセット選択と自然言語調整を追加。
- Sample Performanceから16拍の長尺Note Clipを生成するよう拡張。
- Mute／Solo対応の全パート同時再生、停止、16拍ループを追加。
- 内蔵音源とロード中VST3を同一アレンジ再生から安全に呼び分ける境界を追加。
