...
- 全VST3曲の再生をネイティブトランスポートへ移行。ノート、ループ、フリーズWAVを単一Native Hostのサンプル時計と音声出力へ統合。
- 再生位置をNative Hostのサンプル位置から取得。内蔵音源を含む曲・個別クリップ試聴は従来経路を維持。
- Native Hostの再ビルドとブラウザ再読み込みが必要。
