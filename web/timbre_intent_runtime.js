...
(() => {
  ...
  function ensureIntentPanel(){
    ...
    const style=document.createElement("style");style.id="timbreIntentStyle";style.textContent=`.timbre-intent-panel{margin-top:16px;padding:16px;border:1px solid #39415f;border-radius:16px;background:#111521}.ti-head,.ti-candidate-head{display:flex;justify-content:space-between;gap:14px;align-items:center}.ti-head strong{display:block;font-size:1.05rem}.ti-head small,.ti-help,.ti-status{color:#9ca6c4}.ti-role{padding:8px 12px;border:1px solid #475174;border-radius:999px;color:#dce5ff}.ti-axes{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:9px;margin:14px 0}.ti-axis{display:grid;grid-template-columns:1fr auto;gap:5px 8px;padding:9px;border:1px solid #2e354d;border-radius:11px;background:#0d1018}.ti-axis input{grid-column:1/3;width:100%}.ti-axis output{color:#bcd0ff}.ti-candidates{margin-top:12px;padding-top:12px;border-top:1px solid #2d344b}.ti-candidate-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:9px}.ti-candidate{padding:10px;text-align:left;border:1px solid #343d59;border-radius:12px;background:#151a27}.ti-candidate.active{outline:2px solid #8ea7ff;background:#1c2440}.ti-candidate strong,.ti-candidate span{display:block}.ti-candidate span{color:#9ca6c4;font-size:.78rem;margin-top:3px}.ti-refine{display:grid;grid-template-columns:auto 1fr auto;gap:8px;align-items:center;margin-top:12px}.ti-refine input{min-height:40px;color:#eef1ff;background:#0e1019;border:1px solid #343a56;border-radius:10px;padding:7px 10px}@media(max-width:760px){.ti-candidate-list{grid-template-columns:1fr}.ti-refine{grid-template-columns:1fr}.ti-head{align-items:flex-start;flex-direction:column}}`;
  ...
  function renderCandidates(){
    ...
    state.candidates.forEach((candidate,index)=>{const button=document.createElement("button");button.type="button";button.className=`ti-candidate${index===state.selected?" active":""}`;button.innerHTML=`<strong>${candidate.id} · ${candidate.label}</strong><span>${candidate.description}</span><span>${candidate.patch.instrument_model} / ${candidate.patch.engine_type}</span>`;button.addEventListener("click",()=>applyCandidate(index));root.appendChild(button);});
  }
