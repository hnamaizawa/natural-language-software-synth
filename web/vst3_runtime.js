...
(() => {
  ...
  function renderProgramUi(data){
    ...
    if(!p){
      resetProgramUi("このVST3はホストから切替可能なProgram/Presetパラメータを公開していません。公開パラメータは下の一覧から操作できます。");
      return;
    }
  ...
  async function setProgramIndex(index){
    ...
    try{
      ...
      setTimeout(()=>refreshParameters({preserveProgramMessage:true}).catch(()=>{}),120);
  ...
  async function load(){if(!select.value)return;show("VST3をロード中…");loadBtn.disabled=true;route.checked=false;clearRoutedVisuals();resetProgramUi();
    try{
    ...
    catch(err){loaded=false;route.disabled=true;resetProgramUi("VST3をロードできませんでした。");show(`VST3ロードエラー: ${err.message}`);}finally{loadBtn.disabled=!select.value;}}
  async function unload(){cancelScheduled();route.checked=false;route.disabled=true;try{await api("/api/vst3/unload",{});}catch(_){}loaded=false;unloadBtn.disabled=true;params.innerHTML="";resetProgramUi();show("VST3を解除しました。");}
  ...
  async function refreshParameters({preserveProgramMessage=false}={}){
    ...
    for(const p of visible){const row=document.createElement("label");row.className="vst3-param";const title=document.createElement("span");title.textContent=p.units?`${p.title} (${p.units})`:p.title;const input=document.createElement("input");input.type="range";input.min="0";input.max="1";input.step=p.step_count>1?String(1/p.step_count):"0.001";input.value=String(clamp(p.value,0,1));const value=document.createElement("output");value.textContent=Number(input.value).toFixed(3);let timer=0;input.addEventListener("input",()=>{value.textContent=Number(input.value).toFixed(3);clearTimeout(timer);timer=setTimeout(()=>api("/api/vst3/parameter",{id:p.id,value:Number(input.value)}).catch(err=>show(`VST3パラメータエラー: ${err.message}`)),35);});input.addEventListener("change",()=>input.blur());row.append(title,input,value);params.append(row);}
  ...
  route.addEventListener("change",()=>{if(!route.checked)clearRoutedVisuals();show(route.checked?"VST3ルーティング有効: 鍵盤・PCキー・MIDI・サンプル・鼻歌試聴をVST3へ送ります。":"Web Audio音源へ戻しました。");route.blur();});
  ...
  window["vst3Router"]={scan,load,unload,refreshParameters,testTone,diagnostics,setProgramIndex,isLoaded:()=>loaded,isRouting:()=>loaded&&route.checked};
})();
