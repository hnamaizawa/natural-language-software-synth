...
(() => {
  ...
  async function scan(overridePaths=null){
    show("VST3を検索中…");scanBtn.disabled=true;
    try{
      const paths=Array.isArray(overridePaths)?overridePaths:String(extraScanPaths?.value||"").split(";").map(v=>v.trim()).filter(Boolean);
  ...
  async function trackDiagnostics(track){
    const target=trackInstances.get(track?.id);if(!target)throw new Error("先にこのトラックのVST3をロードしてください");
  ...
  function trackEvents(instanceId,events){
    ...
    const bounded=(events||[]).slice(0,1024).map(event=>({on:Boolean(event.on),note:Math.round(clamp(event.note,0,127)),velocity:clamp(event.velocity,0,1),channel:Math.round(clamp(event.channel,0,15)),delay_ms:clamp(event.delay_ms,0,120000),note_id:Math.round(clamp(event.note_id??-1,-1,0x7ffffffe)),...(event.at_frame?{at_frame:Math.round(clamp(event.at_frame,1,Number.MAX_SAFE_INTEGER))}:{})}));
  ...
  function trackEventsBatch(byTrack,strict=false){const groups=new Map();for(const [trackId,events] of byTrack){const target=trackInstances.get(trackId);if(!target||window.multitrackProject?.isFrozen?.(trackId))continue;const group=groups.get(target.instanceId)||[];group.push(...events);groups.set(target.instanceId,group);}return Promise.all([...groups].map(async([instanceId,events])=>{events.sort((a,b)=>(a.at_frame||a.delay_ms)-(b.at_frame||b.delay_ms));for(let i=0;i<events.length;i+=1024){const data=await api("/api/vst3/events",{instance_id:instanceId,events:events.slice(i,i+1024)});if(!data.ok||data.accepted!==Math.min(1024,events.length-i))throw new Error(data.error||"VST3一括イベント送信失敗");}})).catch(err=>{show(`VST3一括イベント送信エラー: ${err.message}`);if(strict)throw err;});}
  async function scheduleTrackCycle(queue,cycleStart,secondsPerBeat){
    const notes=queue.filter(item=>item.track.source.type==="vst3"&&item.track.source.plugin_id&&!window.multitrackProject?.isFrozen?.(item.track.id));
    if(!notes.length)return true;
    if(notes.length>1024)return false;
    const clock=await api("/api/vst3/status");
    if(!clock.ok||!Number.isFinite(clock.frame_clock)||clock.frame_clock<=0||!Number.isFinite(clock.sample_rate)||clock.sample_rate<8000)return false;
    const rate=clock.sample_rate,origin=clock.frame_clock+Math.max(0,cycleStart-engine.ctx.currentTime)*rate,groups=new Map();
    for(const item of notes){const events=groups.get(item.track.id)||[],channel=channelForTrack(item.track),note_id=nextNoteId();
      events.push({on:true,note:item.event.note,velocity:item.event.velocity*item.track.volume,channel,note_id,delay_ms:0,at_frame:Math.round(origin+item.startBeat*secondsPerBeat*rate)},
        {on:false,note:item.event.note,velocity:0,channel,note_id,delay_ms:0,at_frame:Math.round(origin+item.endBeat*secondsPerBeat*rate)});groups.set(item.track.id,events);}
