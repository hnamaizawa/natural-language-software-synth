...
(() => {
  ...
  async function toggleFreeze(track){
    if(freezingIds.has(track.id))return;
    ...
    try{await engine.init();const buffer=await window.vst3Router.freezeTrack(track,events,(TIMELINE_BEATS*secondsPerBeat+2)*1000);frozenBuffers.set(track.id,buffer);status(`${track.name}をフリーズしました。再生時のVST3演算を休止します。`);}catch(error){status(`フリーズエラー: ${error.message}`);}finally{freezingIds.delete(track.id);renderTrackList();}
  }
  ...
  function addClipFromSample(){
    ...
    if(frozenBuffers.has(track.id)){status("フリーズを解除してからクリップを追加してください。");return;}const clip=performanceToClip(perf,key);track.clips.push(clip);selectedClipId=clip.id;render();status(`「${clip.name}」を${track.name}のNote Clipとして追加しました。`);
  }
  ...
  function startInternalScheduler(queue,onCycleComplete,cycleBeats=TIMELINE_BEATS,scheduledStart=null,loopAhead=false){const runId=++playbackRunId,secondsPerBeat=60/project.bpm,cycleStart=scheduledStart??engine.ctx.currentTime+.08;
    for(const track of new Set(queue.map(item=>item.track))){const buffer=frozenBuffers.get(track.id);if(!buffer)continue;const source=engine.ctx.createBufferSource();source.buffer=buffer;source.connect(engine.master);source.onended=()=>{frozenSources.delete(source);source.disconnect();};frozenSources.add(source);source.start(cycleStart);}
  ...
  async function playArrangement(){
    ...
    const scheduleCycle=(nextStart=null)=>{if(!arrangementPlaying)return;const loop=Boolean(document.getElementById("arrangementLoop")?.checked);startInternalScheduler(queue,cycleEnd=>{if(loop)scheduleCycle(cycleEnd);else{stopPreview();applyTrack(selectedTrack());status("全パートの再生が完了しました。");}},TIMELINE_BEATS,nextStart,loop);};
  ...
  async function importProject(file){const raw=JSON.parse(await file.text());if(!raw||!Array.isArray(raw.tracks)||!raw.tracks.length)throw new Error("tracksを含むProject JSONではありません。");stopPreview();frozenBuffers.clear();await Promise.allSettled(project.tracks.map(track=>window.vst3Router?.releaseTrack?.(track.id)));project={schema_version:SCHEMA_VERSION,name:String(raw.name||"読み込んだ曲").slice(0,60),bpm:Math.round(bounded(raw.bpm,40,240,100)),time_signature:[4,4],selected_track_id:"",tracks:raw.tracks.slice(0,MAX_TRACKS).map(sanitizeTrack)};for(const track of project.tracks){const master=byId(track.source.shared_with);if(!master||master.id===track.id||master.source.type!=="vst3"||master.source.plugin_id!==track.source.plugin_id||master.source.shared_with)delete track.source.shared_with;if(track.source.shared_with)track.midi_channel=master.midi_channel;else if(track.source.type==="vst3"&&track.midi_channel_mode==="auto")allocateVstChannel(track);}project.selected_track_id=project.tracks[0].id;selectedClipId=project.tracks[0].clips[0]?.id||null;applyTrack(project.tracks[0]);render();status(`${project.name}を読み込みました（${project.tracks.length}トラック）。VST3は再生またはトラックのロード時に復元します。`);}
  function bind(){
    ...
    document.getElementById("projectName")?.addEventListener("change",event=>{project.name=String(event.target.value||"新しい曲").slice(0,60);render();});document.getElementById("projectBpm")?.addEventListener("change",event=>{if(frozenBuffers.size){status("BPMを変える前にフリーズを解除してください。");render();return;}project.bpm=Math.round(bounded(event.target.value,40,240,100));render();});
  }
  ...
  window.multitrackProject={trackById:byId,isFrozen:id=>frozenBuffers.has(id),get snapshot(){captureSelectedPatch();return clone(project);},selectTrack,addClipFromSample,playArrangement,exportProject,stopPreview};
})();
