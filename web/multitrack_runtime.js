...
(() => {
  ...
  function showLoadedFilename(){const node=document.getElementById("projectLoadedFile");if(node){node.textContent=`読込ファイル: ${loadedProjectFilename||"なし"}`;node.title=loadedProjectFilename;}}
  ...
  let freezeWorkletContext=null,freezeWorkletReady=null;
  async function recordInternalFreeze(track,queue,durationSeconds){
    const ctx=engine.ctx;
    if(!ctx.audioWorklet||typeof AudioWorkletNode==="undefined"||ctx.sampleRate>96000)throw new Error("このブラウザの音声設定は内蔵音源のフリーズ録音に対応していません。");
    if(freezeWorkletContext!==ctx){freezeWorkletContext=ctx;freezeWorkletReady=ctx.audioWorklet.addModule("/internal_freeze_processor.js");}
    ...
    try{
      const recorded=new Promise((resolve,reject)=>{
        node=new AudioWorkletNode(ctx,"internal-freeze-capture",{numberOfInputs:1,numberOfOutputs:1,outputChannelCount:[2],
          processorOptions:{startFrame:Math.round(start*ctx.sampleRate),endFrame:Math.round(start*ctx.sampleRate)+frames}});
        node.port.onmessage=event=>{if(event.data.done)resolve();else if(event.data.data){const data=new Float32Array(event.data.data);samples+=data.length;chunks.push(data);}};
        node.onprocessorerror=()=>reject(new Error("フリーズ録音の音声処理に失敗しました。"));
      });
      silent=ctx.createGain();silent.gain.value=0;
      engine.master.connect(node);node.connect(silent);silent.connect(ctx.destination);
      const pump=()=>{
        if(!active)return;
        const now=ctx.currentTime,horizon=now+.8;
        engine.suppressPerformanceVisuals=true;
        try{while(cursor<queue.length&&start+queue[cursor].startBeat*secondsPerBeat<=horizon){
          const item=queue[cursor++],delay=Math.max(0,start+item.startBeat*secondsPerBeat-now);
          playNote(track,item.event,true,delay,item.voiceId);
          playNote(track,item.event,false,delay+Math.max(.01,(item.endBeat-item.startBeat)*secondsPerBeat),item.voiceId);
        }}finally{engine.suppressPerformanceVisuals=false;}
        if(cursor<queue.length)timer=setTimeout(pump,Math.max(25,Math.min(100,(start+queue[cursor].startBeat*secondsPerBeat-ctx.currentTime-.8)*1000)));
      };
      pump();
      await recorded;
      if(samples!==frames*2)throw new Error("フリーズ音声の録音長が一致しません。");
      const buffer=ctx.createBuffer(2,frames,ctx.sampleRate),left=buffer.getChannelData(0),right=buffer.getChannelData(1);
  ...
  async function toggleFreeze(track){
    ...
    if(queue.length>512){status("フリーズ対象のノートが多すぎます（最大512ノート）。クリップを分けてください。");return;}
  ...
  function inlineVstSettings(track){
    ...
    const grid=document.createElement("div");grid.className="track-vst-detail-grid";grid.append(picker,scan,channel,load,editor,params,share,state,paramList);details.append(grid);return details;
  }
  function renderTrackList(){
    ...
    for(const track of project.tracks){
      ...
      freeze.addEventListener("click",event=>{event.stopPropagation();toggleFreeze(track);});row.append(freeze);
      row.addEventListener("click",()=>selectTrack(track.id));root.append(row);
    }
  }
  ...
  function renderTrackSoundPanel(){
    ...
    const diag=document.getElementById("trackVstDiagBtn");if(diag)diag.disabled=track.source.type!=="vst3";
  ...
  function prepareInternalSamples(tracks,queue){
    for(const track of tracks){if(track.source.type==="vst3"||frozenBuffers.has(track.id))continue;
      if(track.patch?.instrument_model==="electric_guitar")engine.ensureGuitarSamples?.();
      if(track.patch?.instrument_model==="grand_piano")engine.preparePianoNotes?.(new Set(queue.filter(item=>item.track.id===track.id).map(item=>item.event.note)));
    }
  }
  ...
  function startInternalScheduler(queue,onCycleComplete,cycleBeats=TIMELINE_BEATS,scheduledStart=null,loopAhead=false,frozenOffset=0,activeTracks=[],vstPrescheduled=false){const runId=++playbackRunId,secondsPerBeat=60/project.bpm,cycleStart=scheduledStart??engine.ctx.currentTime+.08;
    const scheduleStem=(buffer,destination)=>{const source=engine.ctx.createBufferSource();source.buffer=buffer;source.connect(destination);source.onended=()=>{frozenSources.delete(source);source.disconnect();};frozenSources.add(source);const offset=frozenOffset*secondsPerBeat,duration=Math.min(cycleBeats*secondsPerBeat,buffer.duration-offset);if(duration>0)source.start(cycleStart,offset,duration);else{frozenSources.delete(source);source.disconnect();}};
  ...
  async function previewClip(){
    ...
    arrangementPlaying=true;const queue=sliceQueue(buildPlaybackQueue([{...track,clips:[clip]}]),clip.start_beats,clip.start_beats+clip.length_beats);prepareInternalSamples([track],queue);const plannedStart=engine.ctx.currentTime+1.2;
  ...
  async function playArrangement(){
    ...
    const scheduleCycle=async(nextStart=null,from=project.playhead_beats)=>{if(!arrangementPlaying)return;const token=playbackRunId,loop=Boolean(document.getElementById("arrangementLoop")?.checked),to=loop?project.loop_end_beats:project.length_beats,begin=from>=to?(loop?project.loop_start_beats:0):from,part=sliceQueue(queue,begin,to),plannedStart=nextStart??engine.ctx.currentTime+1.2;
      ...
      const cycleStart=startInternalScheduler(part,cycleEnd=>{if(loop)scheduleCycle(cycleEnd,project.loop_start_beats);else{project.playhead_beats=0;stopPreview();applyTrack(selectedTrack());renderTransport();status("全パートの再生が完了しました。");}},to-begin,plannedStart,loop,begin,tracks,prescheduled);followPlayhead(begin,to,cycleStart);};
  ...
  async function importProject(file){
    ...
    loadedProjectFilename=String(file.name||"").slice(0,255);showLoadedFilename();project.selected_track_id=project.tracks[0].id;selectedClipId=project.tracks[0].clips[0]?.id||null;applyTrack(project.tracks[0]);render();
    ...
    render();status(`${loadedProjectFilename} を読み込みました（${project.tracks.length}トラック、フリーズ音声${restored.size}パート）。${errors.length?`復元できなかったVST3: ${errors.join("、")}`:"VST3の割当と音色を復元しました。"}`);
  }
  function bind(){
