...
(() => {
  ...
  function buildPlaybackQueue(tracks){const queue=[];for(const track of tracks){for(const clip of track.clips){for(const event of clip.notes){const startBeat=clip.start_beats+event.start_beats;if(startBeat>=project.length_beats||startBeat>=clip.start_beats+clip.length_beats)continue;queue.push({track,event,startBeat,endBeat:Math.min(project.length_beats,clip.start_beats+clip.length_beats,startBeat+event.duration_beats)});}}}return queue.sort((a,b)=>a.startBeat-b.startBeat);}
