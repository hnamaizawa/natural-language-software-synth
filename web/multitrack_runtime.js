...
(() => {
  ...
  function playNote(track,event,on,whenSeconds=0){const router=window.vst3Router;if(track.source.type==="vst3"&&track.source.plugin_id){return on?router?.trackNoteOn(track.id,event.note,event.velocity*track.volume,track.midi_channel,whenSeconds):router?.trackNoteOff(track.id,event.note,track.midi_channel,whenSeconds);}engine.usePreparedPlaybackPatch(preparedPatchFor(track));const fn=on?(router?.baseNoteOn||engine.noteOn.bind(engine)):(router?.baseNoteOff||engine.noteOff.bind(engine));return on?fn(event.note,event.velocity*track.volume,whenSeconds):fn(event.note,whenSeconds);}
  ...
  function startInternalScheduler(queue,onCycleComplete,cycleBeats=TIMELINE_BEATS){const runId=++playbackRunId,secondsPerBeat=60/project.bpm,leadSeconds=.08,cycleStart=engine.ctx.currentTime+leadSeconds;
    const vstEvents=new Map();engine.suppressPerformanceVisuals=true;try{for(const item of queue){const delay=Math.max(0,cycleStart+item.startBeat*secondsPerBeat-engine.ctx.currentTime),duration=Math.max(.01,(item.endBeat-item.startBeat)*secondsPerBeat);if(item.track.source.type==="vst3"&&item.track.source.plugin_id){const events=vstEvents.get(item.track.id)||[];events.push({on:true,note:item.event.note,velocity:item.event.velocity*item.track.volume,channel:item.track.midi_channel,delay_ms:delay*1000},{on:false,note:item.event.note,velocity:0,channel:item.track.midi_channel,delay_ms:(delay+duration)*1000});vstEvents.set(item.track.id,events);}else{playNote(item.track,item.event,true,delay);playNote(item.track,item.event,false,delay+duration);}}for(const [trackId,events] of vstEvents)window.vst3Router?.trackEvents?.(trackId,events);}finally{engine.suppressPerformanceVisuals=false;}
  ...
  function stopPreview(){playbackRunId++;if(completionTimer)clearTimeout(completionTimer);completionTimer=0;if(arrangementPlaying)window.vst3Router?.clearTrackEvents?.();arrangementPlaying=false;const stop=document.getElementById("clipStopBtn");if(stop)stop.disabled=true;}
  ...
  function setTrackSource(type){const track=selectedTrack();if(!track)return;if(type==="vst3"){const plugin=window.vst3Router?.selectedPlugin?.();if(!plugin?.id){status("先にSTEP 2でVST3を検索し、割り当てるプラグインを選択してください。");document.getElementById("soundSource")?.scrollIntoView({behavior:"smooth",block:"start"});renderTrackList();return;}track.source={type:"vst3",plugin_id:plugin.id,name:`VST3: ${plugin.name}`};status(`${track.name}へ${plugin.name}を割り当てました。再生開始時に、このトラック専用VST3インスタンスを準備します。`);}else{window.vst3Router?.releaseTrack?.(track.id);if(type==="reference"){track.source={type:"reference",plugin_id:"",name:"CD/Reference調整済み内蔵音源"};document.getElementById("referenceAudioFile")?.scrollIntoView({behavior:"smooth",block:"center"});status(`${track.name}専用のReference Audio調整を行います。MP3/WAV/M4Aは特徴量解析だけに使い、録音そのものは音源としてコピーしません。`);}else track.source={type:"internal",plugin_id:"",name:"内蔵音源"};}render();}
