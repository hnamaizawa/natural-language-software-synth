...
(() => {
  ...
  function rollLimit(clip){return Math.min(clip.length_beats,TIMELINE_BEATS-clip.start_beats);}
  function renderPianoRoll(){
    const root=document.getElementById("projectPianoRoll"),clip=selectedClip();if(!root)return;
    root.replaceChildren();root.classList.toggle("empty",!clip);if(!clip)return;
    ...
    clip.notes.forEach((note,index)=>{
      ...
      block.style.width=`${Math.max(5,note.duration_beats/TIMELINE_BEATS*width-1)}px`;block.title=`MIDI ${note.note} · ${note.start_beats}拍 · 長さ${note.duration_beats}拍`;
  ...
  async function importProject(file){
    ...
    project={schema_version:SCHEMA_VERSION,name:String(raw.name||"読み込んだ曲").slice(0,60),bpm:Math.round(bounded(raw.bpm,40,240,100)),time_signature:[4,4],selected_track_id:"",tracks:raw.tracks.slice(0,MAX_TRACKS).map(sanitizeTrack)};
