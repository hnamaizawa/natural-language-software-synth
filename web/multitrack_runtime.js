...
(() => {
  ...
  let serial=0,applyingTrack=false,selectedClipId=null,completionTimer=0,playbackTimer=0,playbackRunId=0,arrangementPlaying=false;
  ...
  async function openSelectedVstEditor(){const track=selectedTrack();if(!track||track.source.type!=="vst3"){status("VST3トラックを選択してください。");return;}try{await window.vst3Router.openTrackEditor(track);status(`${track.name}のVST3本体画面を開きました。`);renderTrackSoundPanel();}catch(error){status(`VST3本体画面エラー: ${error.message}`);}}
  function setTrackSource(type,pluginId=""){const track=selectedTrack();if(!track)return;if(type==="vst3"){const plugins=window.vst3Router?.scannedPlugins?.()||[],id=pluginId||document.getElementById("trackVstPluginSelect")?.value||track.source.plugin_id,plugin=plugins.find(item=>item.id===id);if(!plugin){status("このトラックで使うVST3を検索し、一覧から選択してください。");renderTrackList();return;}track.source={type:"vst3",plugin_id:plugin.id,name:`VST3: ${plugin.name}`};allocateVstChannel(track);render();loadSelectedVst(track);return;}else{window.vst3Router?.releaseTrack?.(track.id)?.catch(error=>status(`VST3解除エラー: ${error.message}`));if(type==="reference"){track.source={type:"reference",plugin_id:"",name:"CD/Reference調整済み内蔵音源"};document.getElementById("referenceAudioFile")?.scrollIntoView({behavior:"smooth",block:"center"});status(`${track.name}専用のReference Audio調整を行います。MP3/WAV/M4Aは特徴量解析だけに使い、録音そのものは音源としてコピーしません。`);}else track.source={type:"internal",plugin_id:"",name:"内蔵音源"};}render();}
